            ...
            {sorted.map((it) => {
              ...
              return (
                <div key={it.id} className="flex items-center gap-2.5 p-2 rounded-xl bg-muted/40">
                  ...
                  <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold ${it.review_status === "rejected" ? "bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300" : it.review_status === "pending" ? "bg-amber-100 text-amber-700 dark:bg-amber-950/40 dark:text-amber-300" : isSold ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300" : "bg-muted text-muted-foreground"}`}>
                    {it.review_status === "rejected" ? (ar ? "مرفوض" : "Rejected") : it.review_status === "pending" ? (ar ? "قيد المراجعة" : "Pending") : isSold ? (ar ? "مباع" : "Sold") : (ar ? "متاح" : "Live")}
                  </span>
                </div>
              );
            })}
