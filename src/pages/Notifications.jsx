    ...
    (async () => {
      ...
      try {
        ...
        const notifs = (msgs || [])
          ...
          .map((m) => {
            const r = roomMap.get(m.chatroom_id);
            return {
              id: m.id,
              type: "message",
              text: m.text,
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {items.map((n) => (
            <button
              key={n.id}
              onClick={() => n.type === "message" && nav(`/chat/${n.roomId}`)}
            ...
            >
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${n.type === "message" ? "bg-primary/10 text-primary" : "bg-amber-100 text-amber-600 dark:bg-amber-950 dark:text-amber-300"}`}>
                {n.type === "message" ? <MessageCircle size={18} /> : <Star size={18} className="fill-amber-400 text-amber-400" />}
              </div>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-muted-foreground truncate">
                  {n.text || (n.type === "rating" ? `${t("newRating")} · ${n.score}★` : "")}
                </p>
              </div>
