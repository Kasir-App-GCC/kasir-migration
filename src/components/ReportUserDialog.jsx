...
const USER_REPORT_REASONS = [
  { id: "scam", en: "Scam or fraud", ar: "احتيال أو نصب" },
  { id: "fake_profile", en: "Fake / impersonation profile", ar: "حساب وهمي أو انتحال شخصية" },
  { id: "inappropriate", en: "Inappropriate content or behavior", ar: "محتوى أو سلوك غير لائق" },
  { id: "harassment", en: "Harassment / abusive", ar: "مضايقة أو تعامل سيء" },
  { id: "spam", en: "Spam / repeated postings", ar: "إعلانات مزعجة أو مكررة" },
        ...
        {done ? (
          ...
          <>
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h3 className="font-bold text-lg">{ar ? "بلّغ عن المستخدم" : "Report user"}</h3>
              <button onClick={close} className="p-1.5 rounded-full hover:bg-muted"><X size={20} /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-1.5">
              ...
              {USER_REPORT_REASONS.map((r) => (
                ...
                >
                  ...
                  <span className={`w-4 h-4 rounded-full border-2 ${reason === r.id ? "border-primary bg-primary" : "border-muted-foreground/30"}`} />
                </button>
              ))}
              <textarea
                value={details}
            ...
            <div className="p-4 border-t border-border flex gap-2">
              <button onClick={close} className="px-4 py-3 rounded-xl text-sm font-semibold bg-muted text-muted-foreground">{t("cancel")}</button>
              <button onClick={submit} disabled={!reason || submitting} className="flex-1 py-3 rounded-xl text-sm font-bold bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-50">
                {t("submitReport")}
              </button>
            </div>
          </>
