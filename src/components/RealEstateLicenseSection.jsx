...
export default function RealEstateLicenseSection() {
  ...
  const [editing, setEditing] = useState(status === "" || status === "rejected");
    ...
    <div className="rounded-2xl bg-card border border-border/60 p-4 space-y-3">
      <div className="flex items-center gap-2.5">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold">{ar ? "ترخيص الوساطة العقارية" : "Real Estate Broker License"}</p>
          <p className="text-xs text-muted-foreground">{ar ? "أدخل ترخيصك مرة واحدة لتتمكن من نشر إعلانات عقارية" : "Enter your license once to post real estate listings"}</p>
        </div>
      ...
      {!editing && status === "approved" && (
        <>
          ...
          <button onClick={() => setEditing(true)} className="text-xs text-primary font-semibold hover:underline">{ar ? "تحديث الترخيص" : "Update license"}</button>
        </>
      )}
      ...
      {editing && (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <button onClick={submit} disabled={!valid || submitting} className="flex-1 py-2.5 rounded-xl bg-primary text-primary-foreground font-bold text-sm flex items-center justify-center gap-1.5 disabled:opacity-50">
              {submitting && <Loader2 size={15} className="animate-spin" />}
              {status === "approved" ? (ar ? "إعادة إرسال" : "Re-submit") : (ar ? "إرسال للمراجعة" : "Submit for review")}
            </button>
            {status === "approved" && (
              <button onClick={() => setEditing(false)} className="px-4 py-2.5 rounded-xl bg-muted text-sm font-semibold">{ar ? "إلغاء" : "Cancel"}</button>
            )}
          </div>
        </div>
      )}
